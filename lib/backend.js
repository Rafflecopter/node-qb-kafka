...
module.exports = function (options, qb) {
  ...
  qb.speaks(dialect, options)
    .on('queue-start', function (type, next) {
      ...
      consumer.on('data', 
        _emitTask.bind(null, consumer, type, commitInterval, retryInterval))
...
function _emitTask(consumer, type, commitInterval, retryInterval, message, next) {
  ...
  function __commit($next) {
    return function () {
      if ((offset % commitInterval) !== 0) { return $next() }
