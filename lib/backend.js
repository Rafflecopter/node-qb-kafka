  ...
  qb.speaks(dialect, options)
    .on('queue-start', function (type, next) {
      ...
      client.on('brokersChanged', function () {
        client.refreshMetadata(payloads.getTopics(), function (err) {
      ...
      consumer.on('data', _emitTask.bind(null, type, group, offsetter))
  ...
  function _emitTask(type, group, offsetter, message, next) {
    ...
    function __commit($next) {
      return function () {
        ...
        var cb = retryIfErr(retryInterval, __commit.bind(null, $next), $next)
